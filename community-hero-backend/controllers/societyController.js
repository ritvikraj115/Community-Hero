...
const createSociety = async (req, res) => {
  try {
    ...
    const parsedRadius = Number(radiusInMeters) || 200;

    if (parsedRadius < 50) {
      return res.status(400).json({
        error: 'Community radius must be at least 50 meters.'
      });
    }

    if (normalizedBoundary && !isPointInPolygon(parsedLng, parsedLat, normalizedBoundary)) {
      return res.status(400).json({
        error: 'Your current setup location must be inside the drawn community border.'
      });
    }
...
const requestJoinSociety = async (req, res) => {
  try {
    ...
    if (user.societyId && user.joinStatus === 'approved') {
      return res.status(400).json({ error: 'User is already approved in a society.' });
    }
...
const approveResident = async (req, res) => {
  try {
    ...
    if (
      !resident.lastKnownLocation ||
      resident.lastKnownLocation.latitude === undefined ||
      resident.lastKnownLocation.longitude === undefined
    ) {
      return res.status(400).json({ error: 'Resident must verify location before approval.' });
    }

    const locationCheck = await verifyPointInsideSociety({
      societyId: adminSocietyId,
      latitude: resident.lastKnownLocation.latitude,
      longitude: resident.lastKnownLocation.longitude
    });

    if (!locationCheck.ok) {
      return res.status(locationCheck.status).json({
        error: locationCheck.status === 403
          ? 'Resident verified location is outside this community boundary. Ask them to verify again.'
          : locationCheck.error,
        distanceMeters: locationCheck.distanceMeters,
        radiusInMeters: locationCheck.radiusInMeters
      });
    }
    ...
    resident.lastKnownLocation = {
      latitude: locationCheck.parsedLat,
      longitude: locationCheck.parsedLng,
      verifiedAt: new Date()
    };
    await resident.save();
