...
const ISSUE_IMAGE_MIN_SCORE = Number(process.env.ISSUE_IMAGE_MIN_SCORE || 45);
const ISSUE_IMAGE_REJECTION_CONFIDENCE = Number(process.env.ISSUE_IMAGE_REJECTION_CONFIDENCE || 85);
const RESOLUTION_IMAGE_MIN_SCORE = Number(process.env.RESOLUTION_IMAGE_MIN_SCORE || 55);
const RESOLUTION_IMAGE_REJECTION_CONFIDENCE = Number(process.env.RESOLUTION_IMAGE_REJECTION_CONFIDENCE || 85);
...
const createIssue = async (req, res) => {
  try {
    ...
    let aiResponse = {
      ...
      hasVisibleCivicIssue: true,
      issueConfidenceScore: 50,
    ...
    if (genAI && imagePart) {
      try {
        const reasonSchema = {
          ...
          properties: {
            isImageRelevant: {
              ...
              description:
                'False only when the image is clearly unrelated to the user description.'
            },
            hasVisibleCivicIssue: {
              type: SchemaType.BOOLEAN,
              description:
                'False only when there is clearly no visible civic/infrastructure/sanitation/safety issue in the image.'
            },
            issueConfidenceScore: {
              type: SchemaType.INTEGER,
              description:
                '0-100 score for how clearly the image shows a real reportable civic issue matching the description.'
            },
            decisionConfidence: {
              type: SchemaType.INTEGER,
              description:
                '0-100 confidence in your accept/reject judgment. Use a low value when uncertain.'
            },
            rejectionReason: {
              type: SchemaType.STRING,
              description: 'If clearly invalid/spam/no issue, explain in one sentence. Empty string otherwise.'
            },
        ...
        const prompt = [
          `Analyze this image against the user's civic issue claim: "${description}".`,
          'Reject only when you are highly confident the image is spam, unrelated, or shows no visible civic issue.',
          'If the image is unclear, low quality, or ambiguous, do not make a hard rejection: set decisionConfidence below the rejection threshold and still provide best-effort classification.',
          'Return strict JSON with relevance, visible issue score, decision confidence, category, severityScore, inferredReason, rootCause, risk, tags, and shortExplanation.',
          'Use concise civic infrastructure language.'
        ].join(' ');
    ...
    const issueImageScore = clampScore(aiResponse.issueConfidenceScore, 50);
    const issueDecisionConfidence = clampScore(aiResponse.decisionConfidence, 0);
    const shouldRejectIssueImage =
      issueDecisionConfidence >= ISSUE_IMAGE_REJECTION_CONFIDENCE &&
      (
        aiResponse.isImageRelevant === false ||
        aiResponse.hasVisibleCivicIssue === false ||
        issueImageScore < ISSUE_IMAGE_MIN_SCORE
      );

    if (shouldRejectIssueImage) {
      return res.status(400).json({
        error: 'Image Validation Failed by AI',
        aiReason: aiResponse.rejectionReason || 'The uploaded image does not clearly show a valid community issue.',
        issueConfidenceScore: issueImageScore,
        decisionConfidence: issueDecisionConfidence
      });
    }

    aiResponse.issueConfidenceScore = issueImageScore;
    aiResponse.decisionConfidence = issueDecisionConfidence;
...
const submitResolution = async (req, res) => {
  try {
    ...
    let resolutionDecisionConfidence = 0;
    let resolutionIssueStillPresent = false;
    let resolutionSpamOrIrrelevant = false;
    let resolutionFixValid = null;

    if (genAI && originalPart && resolvedPart) {
      const compareSchema = {
        type: SchemaType.OBJECT,
        properties: {
          confidence_score: { type: SchemaType.INTEGER },
          isFixValid: { type: SchemaType.BOOLEAN },
          issueStillPresent: { type: SchemaType.BOOLEAN },
          isSpamOrIrrelevant: { type: SchemaType.BOOLEAN },
          decisionConfidence: {
            type: SchemaType.INTEGER,
            description: '0-100 confidence in your fix-validity judgment. Use a low value when uncertain.'
          },
      ...
      const prompt = [
        'Compare the original issue image and the resolved/fixed image.',
        'Return confidence_score from 0-100 for whether the problem is genuinely fixed.',
        'Also identify if the original issue is still visibly present, or if the new image is spam/irrelevant/not the same infrastructure.',
        'Reject only when you are highly confident. If uncertain, set decisionConfidence low so community verification can decide.',
        'Consider spoofing, old images, image mismatch, and structural relevance.'
      ].join(' ');

      try {
        const result = await model.generateContent([prompt, originalPart, resolvedPart]);
        const parsed = JSON.parse(result.response.text());
        aiConfidenceScore = clampScore(parsed.confidence_score, 50);
        resolutionDecisionConfidence = clampScore(parsed.decisionConfidence, 0);
        resolutionIssueStillPresent = Boolean(parsed.issueStillPresent);
        resolutionSpamOrIrrelevant = Boolean(parsed.isSpamOrIrrelevant);
        resolutionFixValid = typeof parsed.isFixValid === 'boolean' ? parsed.isFixValid : null;
      ...
      aiConfidenceScore = clampScore(req.body.aiConfidenceScore, 50);
    }

    const shouldRejectResolutionImage =
      resolutionDecisionConfidence >= RESOLUTION_IMAGE_REJECTION_CONFIDENCE &&
      (
        resolutionSpamOrIrrelevant ||
        resolutionIssueStillPresent ||
        resolutionFixValid === false ||
        aiConfidenceScore < RESOLUTION_IMAGE_MIN_SCORE
      );

    if (shouldRejectResolutionImage) {
      return res.status(400).json({
        error: 'Resolution image rejected by AI validation.',
        aiReason: resolutionExplanation || 'The uploaded proof does not confidently show the issue was fixed.',
        aiConfidenceScore,
        decisionConfidence: resolutionDecisionConfidence,
        issueStillPresent: resolutionIssueStillPresent,
        isSpamOrIrrelevant: resolutionSpamOrIrrelevant
      });
    }
    ...
    issue.aiMetadata = {
      ...(issue.aiMetadata || {}),
      latestResolutionImageCheck: {
        aiConfidenceScore,
        decisionConfidence: resolutionDecisionConfidence,
        issueStillPresent: resolutionIssueStillPresent,
        isSpamOrIrrelevant: resolutionSpamOrIrrelevant,
        isFixValid: resolutionFixValid
      }
    };
