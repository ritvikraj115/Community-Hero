...
import { CommandCenterSections } from './DashboardWidgets';
...
const Dashboard = () => {
  ...
  const verifyJoinLocationAgainstSociety = async () => {
    ...
    try {
      ...
      setJoinStatusMsg('Fetching high-accuracy location...');
      const location = await getAccurateLocation({
        onUpdate: (best) => {
          setJoinStatusMsg(`Improving GPS accuracy${formatAccuracy(best.accuracy)}...`);
        }
      });
      ...
      setJoinStatusMsg(
        res.data?.society?.name
          ? `Location verified for ${res.data.society.name}${formatAccuracy(location.accuracy)}.`
          : `Location verified for this society${formatAccuracy(location.accuracy)}.`
      );
    } catch (err) {
      setJoinCoordinates(null);
      setJoinStatusMsg(err.response?.data?.error || err.message || 'Location does not match this society.');
    } finally {
    ...
    try {
      ...
      const res = await api.post(
        '/societies/request-join',
        {
          ...
          locationAccuracy: joinCoordinates.accuracy
        }
      );
      ...
      const updated = {
        ...
        lastKnownLocation: {
          latitude: joinCoordinates.lat,
          longitude: joinCoordinates.lng,
          accuracy: joinCoordinates.accuracy,
  ...
  return user.role === 'admin' ? (
    <div className="dashboard-page">
      <div className="dashboard-hero">
        <div>
          <div className="dashboard-kicker">Operations Command Center</div>
          <h1>Admin Dashboard</h1>
          <p>Create the community, approve residents, monitor AI workflows, and manage civic operations.</p>
        </div>
        <div className="dashboard-hero-actions">{topActions}</div>
      </div>
    ...
    <div className="dashboard-page">
      <div className="dashboard-hero">
        <div>
          <div className="dashboard-kicker">Resident Civic Console</div>
          <h1>Resident Dashboard</h1>
          <p>Join your community, report issues, verify fixes, and track your civic contribution.</p>
        </div>
        <div className="dashboard-hero-actions">{topActions}</div>
      </div>
