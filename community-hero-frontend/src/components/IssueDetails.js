...
const IssueDetails = () => {
  ...
  const claimIssue = async () => {
    try {
      ...
      setMessage(err.response?.data?.error || 'Failed to claim.');
    }
  };
  ...
  const voteApproval = async () => {
    try {
      ...
      setMessage(err.response?.data?.error || 'Failed to vote.');
    }
  };
  ...
  const submitResolution = async (e) => {
    ...
    try {
      ...
      setMessage(err.response?.data?.error || 'Failed to submit resolution.');
    }
  };
  ...
  const requestHelp = async () => {
    try {
      ...
      setMessage(err.response?.data?.error || 'Failed to request help.');
    }
  };
  ...
  const voteResolution = async () => {
    try {
      ...
      setMessage(err.response?.data?.error || 'Failed to vote on resolution.');
    }
  };
