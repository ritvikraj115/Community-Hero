...
const AdminSetup = () => {
  ...
  const focusMapOnLocation = useCallback((location, zoom = 19) => {
  ...
  const handleMapLoad = useCallback((map) => {
    mapRef.current = map;
    if (coordinates) {
      focusMapOnLocation(coordinates, 19);
    }
  }, [coordinates, focusMapOnLocation]);
  ...
  const captureLocation = async () => {
    ...
    try {
      const location = await getAccurateLocation({
        ...
        maxAcceptableAccuracyMeters: 120,
        settleMs: 8000,
        onUpdate: (best) => {
          setStatus(`Improving GPS accuracy${formatAccuracy(best.accuracy)}...`);
        }
      });
      const locked = {
      ...
      focusMapOnLocation(locked, geofenceMode === 'polygon' ? 19 : 18);
      setStatus(`Coordinates locked successfully${formatAccuracy(location.accuracy)}.`);
    } catch (locationErr) {
      setStatus(locationErr.message || 'Please enable location permissions.');
    }
  };

  const handleMapClick = (event) => {
    if (geofenceMode !== 'polygon' || !event.latLng) return;
    ...
    setBoundaryCoordinates((current) => [...current, point]);
  ...
  const handleVertexDrag = (index, event) => {
    ...
    setBoundaryCoordinates((current) => current.map((item, itemIndex) => (
      itemIndex === index ? point : item
    )));
    setStatus('Boundary point adjusted.');
  };

  const undoBoundaryPoint = () => {
    setBoundaryCoordinates((current) => current.slice(0, -1));
    setStatus('Last boundary point removed.');
  };
  ...
  const handleCreateSociety = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/societies', {
        ...
        locationAccuracy: Number(coordinates.accuracy),
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  focusMapOnLocation(coordinates, 19);
                  setStatus('Click the map to add boundary points.');
                }}
              >
                Draw Border
              </button>
          ...
          <div style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ padding: 12, background: '#F8FAFC', color: 'var(--text-muted)', fontWeight: 800 }}>
              {boundaryCoordinates.length >= 3
                ? `${boundaryCoordinates.length} border points selected. Drag points to adjust.`
                : 'Click the map to add at least three community border points.'}
            </div>
            <GoogleMap
            ...
            >
              ...
              {boundaryCoordinates.map((point, index) => (
                <Marker
                  ...
                  draggable
                  label={`${index + 1}`}
                  onDragEnd={(event) => handleVertexDrag(index, event)}
                />
              ))}
              {boundaryCoordinates.length >= 2 && (
                <Polygon
                  paths={boundaryCoordinates}
                  options={{
                    fillColor: '#2563eb',
                    fillOpacity: boundaryCoordinates.length >= 3 ? 0.16 : 0,
                    strokeColor: '#2563eb',
                    strokeWeight: 2,
                    clickable: false
                  }}
            ...
            {boundaryCoordinates.length > 0 && (
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', padding: 12 }}>
                <button type="button" className="btn btn-outline" onClick={undoBoundaryPoint} style={{ width: 'auto' }}>
                  Undo Point
                </button>
                <button type="button" className="btn btn-outline" onClick={() => setBoundaryCoordinates([])} style={{ width: 'auto' }}>
                  Clear Border
                </button>
              </div>
            )}
