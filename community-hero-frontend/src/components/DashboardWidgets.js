...
const formatRain = (rain) => {
  if (!rain) return '-';
  if (typeof rain === 'object') return rain['1h'] ?? rain['3h'] ?? '-';
  return rain;
};
  ...
  return (
    <div className="card" style={{ display: 'grid', gap: 14, alignItems: 'start' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
        ...
        <StatusBadge tone="info">{issues.length} active markers</StatusBadge>
      </div>
      <GoogleMap mapContainerStyle={{ ...mapStyle, height: compact ? '280px' : mapStyle.height }} center={center} zoom={15} options={{ mapTypeControl: false, streetViewControl: false }}>
        ...
        {issues.map((issue) => {
          ...
          return <Marker key={issue._id} position={{ lat, lng }} label="I" onClick={() => { window.location.href = `/issues/${issue._id}`; }} />;
        })}
